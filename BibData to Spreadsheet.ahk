...
;🔍
	;▼ Search by OCLC# via FirstSearch
		if (searchEngine= "FirstSearch"){
			...
			send {enter}
			sleep 3000
		;◯ Loop to check that page has loaded and can search can happen
			loop, 5 {
				sendInput {raw}no:
				sendInput % bibArray[16]
				copyAllclearClip()
				if inStr(clipboard, "no:")
					break
				sleep 2000
			}
			send {enter}
...
;🌐
	;▼ Title search in WorldCat.
		...
		sleep 2000
		send !d
		sendInput {raw}https://www.worldcat.org/search?qt=worldcat_org_all&q=mt`%3abks+not+mt`%3aelc+ti`%3a
		sleep 100
		sendInput % Title
		sleep 100
		send {enter}
...
;🔍
;▼▲▼ Reloads the FirstSearch Page for a fresh search. The cursor returns to the default search box.
	searchFirstSearch(){
		send ^f
		sendInput {raw}Searching
		send {esc}
		send {enter}
		sleep 2000
		send ^{tab 4}
